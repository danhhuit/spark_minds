...
class BookControllerIntegrationTest extends AbstractIntegrationTest {
  ...
  void setUp() throws Exception {
    category = categoryRepository.findByNameIgnoreCase("Technology").orElseThrow();
    ...
    mockMvc
        .perform(
            post("/api/books")
                ...
                .content(createBookJson("978-TEST-0001", "Integration Testing")))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.isbn").value("978-TEST-0001"))
  ...
  @Test
  void invalidBookReturnsValidationErrors() throws Exception {
    mockMvc
        .perform(
            post("/api/books")
                .header(HttpHeaders.AUTHORIZATION, bearer(adminAccessToken))
        ...
        .andExpect(jsonPath("$.fieldErrors.totalQuantity").exists())
        .andExpect(jsonPath("$.fieldErrors.categoryId").exists())
        .andExpect(jsonPath("$.fieldErrors.authorNames").exists());
  }
    ...
    mockMvc
        .perform(
            post("/api/books")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(createBookJson("978-TEST-0002", "Forbidden Book")))
        .andExpect(status().isForbidden());
  }

  @Test
  void userCanSearchBooksWithMaximumTenPerPage() throws Exception {
    seedBooks(12);
  ...
  @Test
  void pageSizeAboveTenReturnsBadRequest() throws Exception {
    mockMvc
        .perform(
            get("/api/books")
                .with(
  ...
  void adminCanUpdateBook() throws Exception {
    Long bookId = createBookThroughApi("978-TEST-0003", "Old Title");
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.authors[0].name").value("Updated Author"));
  }

  @Test
  void deletingBookPerformsSoftDelete() throws Exception {
    Long bookId = createBookThroughApi("978-TEST-0004", "Book To Delete");
  ...
  void duplicateIsbnReturnsConflict() throws Exception {
    createBookThroughApi("978-TEST-0005", "First Book");
    ...
    mockMvc
        .perform(
            post("/api/books")
                .header(HttpHeaders.AUTHORIZATION, bearer(adminAccessToken))
                .contentType(MediaType.APPLICATION_JSON)
                .content(createBookJson("978-TEST-0005", "Duplicate ISBN")))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.message").value("ISBN has already existed"));
  }
  ...
  void missingBookReturnsNotFound() throws Exception {
    mockMvc
        .perform(
            get("/api/books/99999999").header(HttpHeaders.AUTHORIZATION, bearer(adminAccessToken)))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.message").value("Book does not exist: 99999999"));
  }

  @Test
  void userCanLoadCategoryLookup() throws Exception {
    mockMvc
        .perform(
            get("/api/books/lookups/categories")
                .with(
                    jwt()
                        .authorities(
                            new SimpleGrantedAuthority("ROLE_USER"),
  ...
  @Test
  void userCanLoadAuthorLookup() throws Exception {
    createBookThroughApi("978-TEST-LOOKUP", "Lookup Book");

    mockMvc
        .perform(
            get("/api/books/lookups/authors")
                .with(
                    jwt()
                        .authorities(
                            new SimpleGrantedAuthority("ROLE_USER"),
                            new SimpleGrantedAuthority("BOOK_READ"))))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[?(@.name == 'Test Author')]").exists());
  }

  private Long createBookThroughApi(String isbn, String title) throws Exception {
    MvcResult result =
        mockMvc
            .perform(
                post("/api/books")
                    .header(HttpHeaders.AUTHORIZATION, bearer(adminAccessToken))
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(createBookJson(isbn, title)))
            .andExpect(status().isCreated())
            .andReturn();
  ...
  private String createBookJson(String isbn, String title) {
    return """
                {
                  "isbn": "%s",
                  "title": "%s",
                ...
                """
        .formatted(isbn, title, categoryId);
  }
        ...
        authorRepository
            ...
            .orElseGet(
                () -> {
                  ...
                  createdAuthor.setName("Pagination Author");
                  return authorRepository.save(createdAuthor);
                });

    for (int index = 1; index <= count; index++) {
      Book book = new Book();
      book.setIsbn("PAGINATION-%03d".formatted(index));
      book.setTitle("Pagination Book %03d".formatted(index));
