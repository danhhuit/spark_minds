...
@Transactional
class SystemConfigControllerIntegrationTest extends AbstractIntegrationTest {
  ...
  @Test
  void adminCanReadAndUpdateMaintenanceConfiguration() throws Exception {
    mockMvc
        .perform(
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.maintenanceMessage").value("The library is upgrading"))
        .andExpect(jsonPath("$.updatedBy").value("admin"));
  }

  @Test
  void maintenanceModeBlocksBusinessApisButStillAllowsLogin() throws Exception {
    enableMaintenance("Please try again later");

    mockMvc
        .perform(get("/api/books").header(HttpHeaders.AUTHORIZATION, bearer(adminAccessToken)))
  ...
  @Test
  void userCannotManageSystemConfiguration() throws Exception {
    mockMvc
        .perform(
            get("/api/admin/system-config")
  ...
  @Test
  void maintenanceUpdateRequiresEnabledFlag() throws Exception {
    mockMvc
        .perform(
            put("/api/admin/system-config/maintenance")
                .header(HttpHeaders.AUTHORIZATION, bearer(adminAccessToken))
  ...
  private void enableMaintenance(String message) throws Exception {
    mockMvc
        .perform(
            put("/api/admin/system-config/maintenance")
                .header(HttpHeaders.AUTHORIZATION, bearer(adminAccessToken))
