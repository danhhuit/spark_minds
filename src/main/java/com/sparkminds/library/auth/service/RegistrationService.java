...
public class RegistrationService {
  ...
  public MessageResponse register(RegisterRequest request) {
    ...
    verificationToken.setExpiresAt(
        OffsetDateTime.now(ZoneOffset.UTC).plus(timeToLiveProperties.emailVerification()));

    tokenRepository.save(verificationToken);

    mailService.sendVerificationEmail(normalizedEmail, rawToken);
  ...
  private String generateMembershipCode() {
    return "MBR-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase(Locale.ROOT);
  }
