...
public class EmailChangeService {
  ...
  @Transactional
  public MessageResponse requestEmailChange(Jwt jwt, ChangeEmailRequest request) {
    Long userId = getUserId(jwt);

    UserAccount user =
        userAccountRepository
            .findById(userId)
            .orElseThrow(() -> new InvalidEmailChangeException("Account does not exist"));

    String normalizedNewEmail = request.newEmail().trim().toLowerCase(Locale.ROOT);

    if (user.getEmail().equalsIgnoreCase(normalizedNewEmail)) {
      throw new InvalidEmailChangeException("New email must be different " + "from current email");
    }

    ensureEmailAndUsernameAvailable(normalizedNewEmail, userId);

    OffsetDateTime now = OffsetDateTime.now(ZoneOffset.UTC);

    verificationRepository.invalidateUnusedCodes(userId, now);
    ...
    verification.setExpiresAt(now.plus(timeToLiveProperties.emailChangeVerification()));
    verification.setAttempts(0);
    verification.setUsed(false);

    verificationRepository.save(verification);

    mailService.sendEmailChangeCode(normalizedNewEmail, verificationCode);

    return new MessageResponse("Verification code was sent " + "to the new email.");
  }

  @Transactional(noRollbackFor = InvalidEmailChangeException.class)
  public MessageResponse verifyEmailChange(Jwt jwt, VerifyEmailChangeRequest request) {
    Long userId = getUserId(jwt);

    EmailChangeVerification verification =
        verificationRepository
            .findTopByUser_IdAndUsedFalseOrderByCreatedAtDesc(userId)
            .orElseThrow(() -> new InvalidEmailChangeException("No pending email change request"));
    ...
    if (!verification.getExpiresAt().isAfter(now)) {
      ...
      throw new InvalidEmailChangeException("Verification code has expired");
    }
    ...
    if (verification.getAttempts() >= MAX_ATTEMPTS) {
      ...
      throw new InvalidEmailChangeException("Maximum verification attempts exceeded");
    }
    ...
    if (!passwordEncoder.matches(request.code(), verification.getCodeHash())) {
      ...
      throw new InvalidEmailChangeException("Verification code is incorrect");
    }
    ...
    ensureEmailAndUsernameAvailable(newEmail, user.getId());

    // Member username ban đầu chính là email.
    // Admin username "admin" sẽ không bị thay đổi.
    if (user.getUsername().equalsIgnoreCase(oldEmail)) {
    ...
    refreshTokenRepository.revokeAllActiveTokens(user.getId(), now);

    // Access token hiện tại chứa email cũ,
    // vì vậy blacklist để buộc login lại.
    revokedTokenService.revoke(jwt, user.getId());

    return new MessageResponse("Email changed successfully. " + "Please login again.");
  }

  private void ensureEmailAndUsernameAvailable(String newEmail, Long currentUserId) {
    userAccountRepository
    ...
    userAccountRepository
        ...
        .ifPresent(
            user -> {
              throw new ResourceAlreadyExistsException("Email cannot be used");
            });
  }
  ...
  private Long getUserId(Jwt jwt) {
    ...
    if (userIdClaim == null) {
      throw new InvalidEmailChangeException("Authenticated user is invalid");
    }
