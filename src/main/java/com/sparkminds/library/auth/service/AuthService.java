...
public class AuthService {
  ...
  public TokenResponse login(LoginRequest request) {
    ...
    RefreshTokenService.IssuedRefreshToken refresh =
        refreshTokenService.issueForUser(principal.getId());

    return createResponse(principal, refresh.value(), refresh.expiresAt());
  }
  ...
  public TokenResponse refresh(String rawRefreshToken) {
    ...
    CustomUserPrincipal principal = CustomUserPrincipal.from(rotated.user());

    return createResponse(principal, rotated.value(), rotated.expiresAt());
  }
  ...
  public void logout(Jwt jwt, String rawRefreshToken) {
    ...
    refreshTokenService.revokeIfPresent(rawRefreshToken, userId);

    revokedTokenService.revoke(jwt, userId);
    accessTokenService.revoke(jwt.getId(), userId);
  }
  ...
  private TokenResponse createResponse(
    ...
    GeneratedAccessToken accessToken = jwtTokenService.generateAccessToken(principal);
    accessTokenService.record(
        principal.getId(), principal.getAuthorizationVersion(), accessToken);

    long expiresIn =
        Math.max(0, Duration.between(Instant.now(), accessToken.expiresAt()).toSeconds());
