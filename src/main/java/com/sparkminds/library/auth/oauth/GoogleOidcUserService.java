...
public class GoogleOidcUserService {
  ...
  //
  private OAuthIdentity createOrLinkIdentity(OidcUser oidcUser, String subject, String email) {
    UserAccount user =
        userAccountRepository
            .findByEmailIgnoreCase(email)
            .orElseGet(() -> createGoogleUser(oidcUser, email));

    if (identityRepository.existsByUser_IdAndProvider(user.getId(), PROVIDER)) {
      throw new BusinessException(
          "This library account is already " + "linked to another Google account");
    }
  ...
  private UserAccount createGoogleUser(OidcUser oidcUser, String email) {
    Role userRole =
        roleRepository
    ...
    user.setEmail(email);
    user.setPassword(passwordEncoder.encode(UUID.randomUUID() + "@Google1"));
    user.setEnabled(true);
    user.setEmailVerified(true);
    user.setAccountNonLocked(true);
    user.addRole(userRole);
    // tạo hồ sơ thành viên mới cho người dùng Google, mã thành viên được tạo tự động với tiền tố
    // "MBR-" và 8 ký tự ngẫu nhiên
  ...
  private String resolveUsername(String email) {
    if (!userAccountRepository.existsByUsernameIgnoreCase(email)) {
      return email;
    }

    return "google_" + UUID.randomUUID().toString().substring(0, 12);
  }
