...
public class ProfileService {
  ...
  public ProfileResponse updateCurrentProfile(Long userId, UpdateProfileRequest request) {
    ...
    } else if (!hasAdministrativeRole(user)) {
  ...
  private UserAccount getUser(Long userId) {
    return userAccountRepository
        ...
        .orElseThrow(() -> new ResourceNotFoundException("User account does not exist: " + userId));
  }

  private void validateUsernameAvailable(String username, Long userId) {
    boolean collidesWithUsername =
        userAccountRepository.existsByUsernameIgnoreCaseAndIdNot(username, userId);

    boolean collidesWithEmail =
        userAccountRepository.existsByEmailIgnoreCaseAndIdNot(username, userId);

    if (collidesWithUsername || collidesWithEmail) {
      throw new ResourceAlreadyExistsException("Username is already in use");
    }
  }

  private void updateProfile(MemberProfile profile, UpdateProfileRequest request) {
  ...
  private ProfileResponse toResponse(UserAccount user) {
    ...
    return new ProfileResponse(
        ...
        profile == null ? null : profile.getMembershipCode(),
        ...
        profile != null && profile.getPhone() != null && profile.getDateOfBirth() != null);
  }
  ...
  private boolean hasAdministrativeRole(UserAccount user) {
