...
public class BorrowingMapper {
  ...
  public BorrowingResponse toResponse(Borrowing borrowing) {
    ...
    boolean overdue =
        borrowing.getStatus() == BorrowingStatus.BORROWED
            && borrowing.getDueAt().isBefore(OffsetDateTime.now(ZoneOffset.UTC));
  ...
  private String resolveMemberName(Borrowing borrowing) {
    ...
    String username = borrowing.getMember().getUser().getUsername();

    if (username != null && !username.isBlank()) {
      return username;
    }

    return borrowing.getMember().getUser().getEmail();
  }
}
