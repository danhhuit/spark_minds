...
public class MemberMapper {
  ...
  public MemberResponse toResponse(MemberProfile profile) {
    List<String> roles =
        profile.getUser().getRoles().stream().map(role -> role.getName().name()).sorted().toList();
    ...
    return new MemberResponse(
        ...
        profile.getUser().getPassword() != null && !profile.getUser().getPassword().isBlank(),
        roles,
